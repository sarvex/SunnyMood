...
public class Stage4_3_Tests
{
    ...
    public IEnumerator NecessaryComponents()
    {
        ...
        foreach (var clipName in new[] {"Idle", "Jump", "Walk"})
        {
            ...
            if (!clip) Assert.Fail(" Level 2: There should be a clip in \"Player\"'s animator, called \""+clipName+"\"");
            if (clip.legacy)
                Assert.Fail(" Level 2: \""+clipName+"\" clip should be animated by animator, not by the <Legacy Animation>" +
                            " component, so it's legacy property should be unchecked");
            if (clip.empty) Assert.Fail(" Level 2: \""+clipName+"\" clip in Player's animator should have animation keys");
            if (!clip.isLooping) Assert.Fail(" Level 2: \""+clipName+"\" clip in Player's animator should be looped");
        }
    ...
    public IEnumerator NecessaryGemComponents()
    {
        ...
        if (gems.Length != 3)
        {
            Assert.Fail(" Level 2: There should be exactly three gems in scene");
        }
        ...
        {
            Assert.Fail(" Level 2: There should be added only 1 clip to Gem's animator, called \"Gem\"");
        }
